...
// 设置资源保存路径
files.createWithDirs("/sdcard/星月书/");
// fInfo("运行前重置学习APP");
// exit_app("学习强国");
// sleep(1500);
// 检测地理位置权限代码，出现就点掉
...
function get_tiku_by_http(link) {
  ...
  // 更新题库时若获取不到，则文件名+1
  if (req.statusCode != 200) {
    throw "网络原因未获取到题库，请尝试切换流量或者更换114DNS，退出功能";
    return false;
  }
...
function init_wh() {
  ...
  if (device.width == device_h && device.height == device_w) {
    fError("设备屏幕方向检测为横向，后续运行很可能会报错，建议调整后重新运行功能");
    sleep(10000);
  }
  else if (device.width == 0 || device.height == 0) {
    fError("识别不出设备宽高，建议重启hamibot后重新运行功能");
    sleep(10000);
  }
...
function ocr_test() {
  try {
    ...
    if (test_time > 10000) {
      fError("OCR识别过慢(>10s)，建议更换Pro版功能选择第三方OCR");
      sleep(3000);
      return true;
    } else {
      fInfo("OCR功能正常");
...
var jifen_map = {
  "评论": 10, "视频": 2, "文章": 1, "每日": 4, "每周": 13, "专项": 5, "挑战": 6, "四人": 7, "双人": 8,
  "订阅": 9, "本地": 11
}
fSet("username", name);
