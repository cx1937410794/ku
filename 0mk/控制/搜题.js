...
function 查题(题目) {
  ...
  threads.start(function () {
    let res = http.get("https://zuoxiaozi.top/0mk/控制/项目题库.json", { headers: { 'Accept-Language': 'zh-cn,zh;q=0.5', 'User-Agent': 'Mozilla/5.0(Macintosh;IntelMacOSX10_7_0)AppleWebKit/535.11(KHTML,likeGecko)Chrome/17.0.963.56Safari/535.11' } });
...
function checkid() { //检查id函数
  ...
  if (id("tvID").className("android.widget.TextView").findOne(3000) != null) { //获取个人id，如果没有登录则返回null
    ...
    threads.start(function () {
      function 到期时间() {
        let res = http.get("https://zuoxiaozi.top/0mk/UI/右侧账号.json", {
          headers: {
